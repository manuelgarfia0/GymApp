...
class _ActiveWorkoutScreenState extends State<ActiveWorkoutScreen> {
  ...
  void initState() {
    ...
    if (widget.previousWorkout != null && widget.baseRoutine != null) {
      ...
      for (var exId in uniqueExerciseIds) {
        ...
        int savedRest = 90;
        try {
          var routineEx = widget.baseRoutine!.exercises.firstWhere((e) => e.exerciseId == exId);
          if (routineEx.restSeconds > 0) savedRest = routineEx.restSeconds;
        } catch (e) {}

        // Recuperamos la nota antigua si existe (la cogemos del primer set)
        ...
        _activeExercises.add(ActiveExercise(exercise: mockExercise, sets: initialSets, restSeconds: savedRest, notes: pastNotes));
      }
      ...
      for (var routineEx in widget.baseRoutine!.exercises) {
        ...
        _activeExercises.add(ActiveExercise(exercise: mockExercise, sets: initialSets, restSeconds: savedRest));
      }
    } else {
      for (var ex in widget.selectedExercises) {
        _activeExercises.add(ActiveExercise(exercise: ex, sets: [ActiveSet()], restSeconds: 90));
      }
    }
  ...
  void _toggleSetCompletion(int exerciseIndex, int setIndex) {
    setState(() {
      bool wasCompleted = _activeExercises[exerciseIndex].sets[setIndex].isCompleted;
      _activeExercises[exerciseIndex].sets[setIndex].isCompleted = !wasCompleted;
        ...
        return StatefulBuilder(
          builder: (context, setDialogState) {
            return AlertDialog(
              ...
              title: const Text('Set Rest Timer', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
              content: Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  const Text('Select rest duration for this exercise', style: TextStyle(color: Colors.grey)),
                  ...
                  Row(
                    ...
                    children: [
                      Column(
                        children: [
                          const Text('MIN', style: TextStyle(color: Colors.blueAccent, fontWeight: FontWeight.bold)),
                          DropdownButton<int>(
                            dropdownColor: const Color(0xFF2C2C2C),
                            value: selectedMinutes,
                            items: List.generate(11, (i) => DropdownMenuItem(value: i, child: Text('$i', style: const TextStyle(color: Colors.white, fontSize: 20)))),
                      ...
                      Column(
                        children: [
                          const Text('SEC', style: TextStyle(color: Colors.blueAccent, fontWeight: FontWeight.bold)),
                          DropdownButton<int>(
                            dropdownColor: const Color(0xFF2C2C2C),
                            value: selectedSeconds,
                            items: [0, 15, 30, 45].map((i) => DropdownMenuItem(value: i, child: Text(i.toString().padLeft(2, '0'), style: const TextStyle(color: Colors.white, fontSize: 20)))).toList(),
                            onChanged: (val) => setDialogState(() => selectedSeconds = val!),
                          ),
                        ],
                      ),
                    ],
              ...
              actions: [
                ...
                ElevatedButton(
                  ...
                  onPressed: () {
                    setState(() {
                      _activeExercises[exerciseIndex].restSeconds = (selectedMinutes * 60) + selectedSeconds;
                    });
  ...
  // ¡NUEVO! Dialog para escribir una nota en un ejercicio
  Future<void> _showNotesDialog(int exerciseIndex) async {
    TextEditingController noteController = TextEditingController(text: _activeExercises[exerciseIndex].notes);
    ...
    await showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          backgroundColor: const Color(0xFF1E1E1E),
          title: Text('Notes - ${_activeExercises[exerciseIndex].exercise.name}', style: const TextStyle(color: Colors.white, fontSize: 18)),
          content: TextField(
          ...
          actions: [
            ...
            ElevatedButton(
              ...
              onPressed: () {
                setState(() {
                  _activeExercises[exerciseIndex].notes = noteController.text.trim();
                });
                Navigator.pop(context);
              },
              child: const Text('SAVE', style: TextStyle(color: Colors.white)),
    ...
    return await showDialog<bool>(
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: const Text('Are you sure you want to cancel this workout? All progress will be lost.', style: TextStyle(color: Colors.white70)),
          actions: [
            TextButton(
    ...
    return await showDialog<bool>(
      ...
      builder: (context) {
        return AlertDialog(
          ...
          content: const Text('Are you ready to complete and save this workout?', style: TextStyle(color: Colors.white70)),
          actions: [
            TextButton(
              onPressed: () => Navigator.pop(context, false), 
  ...
  Future<void> _attemptFinishWorkout() async {
      ...
      for (var s in activeEx.sets) {
        if (s.isCompleted && s.weight != null && s.reps != null) {
          ...
          setsToSend.add(WorkoutSetDTO(
            ...
            weight: s.weight!,
            reps: s.reps!,
            notes: setNumber == 1 ? activeEx.notes : null, // Guardamos la nota en la primera serie para que viaje a la BD
          ));
          setNumber++;
        }
      }
    ...
    final DateTime realStartTime = DateTime.now().subtract(Duration(seconds: _workoutSecondsElapsed));

    final request = WorkoutDTO(
      name: finalName,
      startTime: realStartTime.toIso8601String(),
        ...
        appBar: AppBar(
          ...
          title: Column(
            ...
            children: [
              ...
              Text(_formatTime(_workoutSecondsElapsed), style: const TextStyle(fontSize: 14, color: Colors.white70)),
            ],
          ),
                            ...
                            children: [
                              Row(
                                children: [
                                  ...
                                  // ¡NUEVO! Botón para las Notas
                                  IconButton(
                                    icon: Icon(
                                      Icons.notes, 
                                      color: activeEx.notes.isNotEmpty ? Colors.amber : Colors.grey, // Si hay nota escrita, brilla en dorado
                                    ),
                                    onPressed: () => _showNotesDialog(exerciseIndex),
                                  ),

                                  // Botón de descanso
                                  InkWell(
                                    onTap: () => _showRestTimerPicker(exerciseIndex),
                                    child: Container(
                                      ...
                                      child: Row(
                                        children: [
                                          ...
                                          Text(
                                            _formatRestTime(activeEx.restSeconds),
                                  ...
                                  IconButton(
                                    ...
                                    onPressed: () {
                                      setState(() { _activeExercises.removeAt(exerciseIndex); });
                                    },
                                  ),
                              ...
                              // ¡NUEVO! Si hay una nota guardada, la mostramos debajo del título del ejercicio
                              if (activeEx.notes.isNotEmpty)
                                Padding(
                                  ...
                                  child: Container(
                                    ...
                                    child: Text(
                                      activeEx.notes,
                                      style: const TextStyle(color: Colors.amber, fontSize: 13, fontStyle: FontStyle.italic),
                                    ),
                                  ),
                                ),
                              ...
                              ...List.generate(activeEx.sets.length, (setIndex) {
                                ...
                                return Dismissible(
                                  ...
                                  onDismissed: (direction) {
                                    setState(() { activeEx.sets.removeAt(setIndex); });
                                  },
                                  child: Padding(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        Column(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            const Text('REST', style: TextStyle(color: Colors.white70, fontSize: 12, fontWeight: FontWeight.bold)),
                            Text(
