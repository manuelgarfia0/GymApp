...
class _CreateRoutineScreenState extends State<CreateRoutineScreen> {
  ...
  void dispose() {
    ...
    if (selectedExercise != null) {
      // ¡NUEVO! Comprobamos si el ejercicio ya está en la lista
      if (_exercises.any((e) => e.exercise.id == selectedExercise.id)) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('${selectedExercise.name} is already in the routine.'), 
  ...
  Future<void> _saveRoutine() async {
    ...
    if (name.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please enter a routine name'), backgroundColor: Colors.orange),
      );
      return;
    }
    ...
    if (_exercises.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Add at least one exercise'), backgroundColor: Colors.orange),
      );
      return;
    }
    ...
    for (int i = 0; i < _exercises.length; i++) {
      exerciseDTOs.add(RoutineExerciseDTO(
        exerciseId: _exercises[i].exercise.id,
        orderIndex: i + 1, // Guardamos el orden correcto que el usuario eligió
    ...
    final success = await _routineService.saveRoutine(newRoutine);
    ...
    if (success) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Routine created successfully!'), backgroundColor: Colors.green),
        );
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Failed to create routine'), backgroundColor: Colors.red),
        );
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Create Routine', style: TextStyle(fontWeight: FontWeight.bold)),
        ...
        children: [
          ...
          Expanded(
            child: ReorderableListView.builder(
              buildDefaultDragHandles: false, // Igual que antes, icono manual
              ...
              itemBuilder: (context, index) {
                ...
                return Card(
                  key: Key(draftEx.id), // ¡VITAL! Para el orden
                      ...
                      children: [
                        ...
                        Row(
                          ...
                          children: [
                            Column(
                              children: [
                                const Text('TARGET SETS', style: TextStyle(color: Colors.grey, fontSize: 12, fontWeight: FontWeight.bold)),
                            ...
                            Column(
                              children: [
                                const Text('TARGET REPS', style: TextStyle(color: Colors.grey, fontSize: 12, fontWeight: FontWeight.bold)),
                                const SizedBox(height: 8),
                                SizedBox(
                                  width: 80,
                                  child: TextFormField(
                                    ...
                                    onChanged: (value) {
                                      draftEx.targetReps = int.tryParse(value) ?? 1;
                                    },
                                  ),
                                ),
                              ],
